...
export const useOnchainStore = create<OnchainState & OnchainActions>()(
  persist(
    ...
    }),
    {
      ...
      // Don't persist large data structures, only essential data
      partialize: (state) => ({
      ...
      // Add data validation on rehydration
      onRehydrateStorage: () => (state) => {
        if (state) {
          // Validate data freshness (if older than 5 minutes, consider stale)
          const now = Date.now();
          const dataAge = now - (state.lastUpdate || 0);
