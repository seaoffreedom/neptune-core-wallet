...
function SettingsLayout() {
  ...
  const countChanges = useCallback(
    (current: NeptuneCoreSettings, original: NeptuneCoreSettings): number => {
      let count = 0;
      ...
      for (const category of categories) {
        ...
        if (currentCategory && originalCategory) {
          ...
          for (const key of allKeys) {
            const currentValue =
              currentCategory[key as keyof typeof currentCategory];
            const originalValue =
  ...
  const handleReset = async () => {
    ...
    logger.info('Reloading settings from storage...');
    ...
    logger.info('Settings reset to last saved state');
  };

  logger.debug('SaveChangesBar state', {
    hasChanges,
    changeCount,
    isSaving,
    needsRestart,
