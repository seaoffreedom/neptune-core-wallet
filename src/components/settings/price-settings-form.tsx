...
import { useSelectedCurrency, useSetCurrency, AVAILABLE_CURRENCIES } from '@/store/ui.store';
import { usePricePolling, useIsPriceCacheValid, useCacheExpiryTime } from '@/hooks/use-price-polling';
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                    <div className="space-y-0.5">
                      ...
                      <FormDescription>
                        Automatically fetch Neptune prices from CoinGecko API
                      </FormDescription>
                    </div>
                    <FormControl>
                      <Switch
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Currency to display for price conversions
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      How often to refresh price data from CoinGecko
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <span className={`ml-2 font-medium ${isPollingActive ? 'text-green-600' : 'text-orange-600'}`}>
                  {isPollingActive ? 'Active' : 'Inactive'}
                </span>
              </div>
              <div>
                <span className="text-muted-foreground">Cache:</span>
                <span className={`ml-2 font-medium ${isCacheValid ? 'text-green-600' : 'text-red-600'}`}>
            ...
            {priceFetchingSettings.lastFetched && (
              <div className="text-sm">
                ...
                <span className="ml-2">
                  {new Date(priceFetchingSettings.lastFetched).toLocaleString()}
                </span>
              </div>
            )}
            ...
            {cacheExpiryTime && (
              <div className="text-sm">
                <span className="text-muted-foreground">Cache Expires:</span>
                <span className="ml-2">
