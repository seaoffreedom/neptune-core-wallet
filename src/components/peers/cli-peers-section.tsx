...
export function CliPeersSection() {
  ...
  const handleAddPeer = async (cliPeer: CliPeer) => {
    ...
    try {
      ...
      toast.error(`Failed to add peer: ${(error as Error).message}`);
    } finally {
    ...
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">
          Peers discovered via Neptune CLI. Click "Add to Store" to permanently
          save them.
        </p>
      </CardHeader>
      <CardContent>
        {cliPeers.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-muted-foreground">No connected peers found</p>
            <p className="text-sm text-muted-foreground mt-1">
              Make sure Neptune Core is running and connected to the network
            </p>
          </div>
        ) : (
          <Table>
            ...
            <TableBody>
              {cliPeers.map((peer) => {
                ...
                const isAdding =
                  isAddingPeer === `${peer.address}:${peer.port}`;

                return (
                  <TableRow key={`${peer.address}:${peer.port}`}>
                    <TableCell className="font-mono">{peer.address}</TableCell>
                    <TableCell className="font-mono">{peer.port}</TableCell>
                    <TableCell>
                      <Badge
                        variant={peer.connected ? 'default' : 'secondary'}
