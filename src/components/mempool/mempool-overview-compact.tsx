...
export function MempoolOverviewCompact({
  ...
  const handleBroadcastAll = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast.success('All mempool transactions broadcasted successfully', {
          description:
            result.result || 'Transactions have been sent to the network',
        });
        onRefresh(); // Refresh data after broadcasting
      } else {
        toast.error('Failed to broadcast transactions', {
          description: result.error || 'Unknown error occurred',
        });
      }
    } catch (error) {
      toast.error('Error broadcasting transactions', {
  ...
  const handleClearMempool = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast.success('Mempool cleared successfully', {
          description: result.result || 'All transactions removed from mempool',
        });
        onRefresh(); // Refresh data after clearing
      } else {
        toast.error('Failed to clear mempool', {
          description: result.error || 'Unknown error occurred',
        });
      }
    } catch (error) {
      toast.error('Error clearing mempool', {
        description: (error as Error).message,
      });
    } finally {
      setIsClearing(false);
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-orange-500">{ownCount}</div>
            <div className="text-xs text-muted-foreground">Your TXs</div>
          </div>
          <div className="text-center">
        ...
        {transactions.length > 0 && (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 gap-2">
              {['PrimitiveWitness', 'ProofCollection', 'SingleProof'].map(
                (type) => {
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-muted-foreground">
                            {typeInfo.status} • {typeInfo.fee} fees
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                          ...
                          <Tooltip>
                            ...
                            <TooltipContent className="max-w-xs">
                              <div className="space-y-1">
                                ...
                                <div className="text-xs">
                                  <strong>Can Update:</strong>{' '}
                                  {typeInfo.canUpdate ? 'Yes' : 'No'}
                                </div>
                              </div>
                            </TooltipContent>
                          </Tooltip>
              ...
              <Table>
                        ...
                        <TooltipProvider>
                          <Tooltip>
                            ...
                            <TooltipContent className="max-w-xs">
                              <div className="space-y-1">
                                <div className="font-medium">
                                  Transaction Proof Types
                                </div>
                                <div className="text-xs">
                                  <strong>Primitive Witness:</strong> Draft
                                  transaction (local only)
                                </div>
                                <div className="text-xs">
                                  <strong>Proof Collection:</strong> Ready for
                                  upgrading (higher fees)
                                </div>
                                <div className="text-xs">
                                  <strong>Single Proof:</strong> Ready for
                                  blocks (lowest fees)
                                </div>
                              </div>
                            </TooltipContent>
                ...
                <TableBody>
                  {recentTransactions.map((tx, index) => {
                    ...
                    const isOwn = tx.id?.startsWith('own_') || false;
                    ...
                    return (
                      <TableRow key={tx.id} className="group hover:bg-muted/50">
                        ...
                        <TableCell className="text-sm">
                          {tx.num_inputs || 0} → {tx.num_outputs || 0}
                        </TableCell>
                        ...
                        <TableCell>
                          <Badge
                            variant={
                              tx.synced === true ? 'default' : 'secondary'
                            }
                          ...
                          >
                            ...
                            />
                            {tx.synced === true ? 'Synced' : 'Pending'}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-sm">
                          {(positiveEffect > 0 || negativeEffect > 0) && (
                            <div className="space-y-1">
                              {positiveEffect > 0 && (
                                <div className="flex items-center gap-1">
                                  <div className="w-1.5 h-1.5 rounded-full bg-green-500" />
                                  <span className="font-mono text-green-600 text-xs">
                                    +{positiveEffect.toFixed(2)} $NPT
                                  </span>
                                </div>
                              )}
                              {negativeEffect > 0 && (
