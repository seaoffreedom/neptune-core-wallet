...
export function registerBlockchainHandlers(): void {
  ...
  ipcMain.handle(IPC_CHANNELS.BLOCKCHAIN_GET_BALANCE, async () => {
    try {
      const [confirmed, unconfirmed] = await Promise.all([
        neptuneRpcService.getConfirmedAvailableBalance(),
        neptuneRpcService.getUnconfirmedAvailableBalance(),
      ]);
      return {
        success: true,
        confirmed,
  ...
  ipcMain.handle(IPC_CHANNELS.BLOCKCHAIN_GET_PEER_INFO, async () => {
    try {
      const peers = await neptuneRpcService.getPeerInfo();
  ...
  ipcMain.handle(
    ...
    async (_event, params: unknown) => {
      try {
        const overview = await neptuneRpcService.getMempoolOverview(
          params as Parameters<typeof neptuneRpcService.getMempoolOverview>[0]
        );
    ...
    async (_event, params: unknown) => {
      try {
        const isValid = await neptuneRpcService.validateAddress(
          params as Parameters<typeof neptuneRpcService.validateAddress>[0]
        );
    ...
    async (_event, params: unknown) => {
      try {
        const isValid = await neptuneRpcService.validateAmount(
          params as Parameters<typeof neptuneRpcService.validateAmount>[0]
        );
        return { success: true, isValid };
      } catch (error) {
        return {
