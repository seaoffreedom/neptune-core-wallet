...
const walletStorage = new Map<string, any>();
...
export async function handleWalletCreate(
  ...
  try {
    ...
    if (request.path) {
      const walletPath = path.join(request.path, `${request.name}.wallet`);
    ...
    logger.error({ error: (error as Error).message }, 'Error creating wallet');
    return {
      success: false,
...
export async function handleWalletLoad(
  ...
  try {
    ...
    logger.error({ error: (error as Error).message }, 'Error loading wallet');
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error',
    };
...
export async function handleWalletSave(
  ...
  try {
    ...
    // Save wallet to file
    await fs.writeFile(request.path, JSON.stringify(wallet, null, 2), 'utf8');

    return {
      success: true,
    };
  } catch (error) {
    logger.error({ error: (error as Error).message }, 'Error saving wallet');
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error',
    };
