...
export async function readFileWithRetry(
  ...
  return pRetry(
    () =>
      pTimeout(fs.readFile(filePath, encoding), {
...
export async function readJsonWithRetry<T = unknown>(
  ...
  return pRetry(
    () => pTimeout(fs.readJson(filePath), { milliseconds: timeout }),
    {
      retries,
      factor: 2,
      minTimeout: 1000,
...
export async function fileExistsWithRetry(
  ...
  try {
    await pRetry(
      () => pTimeout(fs.access(filePath), { milliseconds: timeout }),
      {
        retries,
        factor: 2,
        minTimeout: 500,
...
export async function ensureDirWithRetry(
  ...
  return pRetry(
    () => pTimeout(fs.ensureDir(dirPath), { milliseconds: timeout }),
    {
      retries,
      factor: 2,
      minTimeout: 1000,
