...
export async function readFileWithRetry(
    ...
    return pRetry(() => pTimeout(fs.readFile(filePath, encoding), timeout), {
        retries,
        factor: 2,
        minTimeout: 1000,
        maxTimeout: 3000,
...
export async function readJsonWithRetry<T = unknown>(
    ...
    return pRetry(() => pTimeout(fs.readJson(filePath), timeout), {
        retries,
        factor: 2,
        minTimeout: 1000,
        maxTimeout: 3000,
...
export async function fileExistsWithRetry(
    ...
    try {
        await pRetry(() => pTimeout(fs.access(filePath), timeout), {
            retries,
            factor: 2,
            minTimeout: 500,
            maxTimeout: 2000,
...
export async function ensureDirWithRetry(
    ...
    return pRetry(() => pTimeout(fs.ensureDir(dirPath), timeout), {
        retries,
        factor: 2,
        minTimeout: 1000,
        maxTimeout: 3000,
