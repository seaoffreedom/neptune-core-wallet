...
export class AsyncValidationCircuitBreaker {
    ...
    async validate<T>(
        ...
        if (this.state === 'open') {
            if (Date.now() - this.lastFailureTime > (this.options.resetTimeout || 30000)) {
                ...
                logger.info({ context: this.options.context }, "Circuit breaker transitioning to half-open");
            } else {
        ...
        try {
            ...
            if (this.state === 'half-open') {
                this.state = 'closed';
                this.failures = 0;
                logger.info({ context: this.options.context }, "Circuit breaker closed after successful validation");
            }
    ...
    reset() {
        ...
        this.state = 'closed';
        logger.info({ context: this.options.context }, "Circuit breaker manually reset");
    }
}
