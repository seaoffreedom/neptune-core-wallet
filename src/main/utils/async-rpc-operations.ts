...
export class RpcConnectionHealthMonitor {
  ...
  isConnectionHealthy(): boolean {
    ...
    if (!this.isHealthy) {
      ...
      if (timeSinceLastFailure > (this.options.healthCheckInterval || 10000)) {
        this.isHealthy = true;
        this.consecutiveFailures = 0;
        logger.info(
          { context: this.options.context },
...
export class ResilientRpcOperation {
  ...
  async execute<T>(
    ...
    if (deduplicateKey) {
      return this.deduplicator.deduplicate(deduplicateKey, executeOperation);
    }
  ...
  async executeBatch<T>(
    ...
    const { retries = 3, timeout = 10000, skipHealthCheck = false } = options;
