...
export class NeptuneCoreArgsBuilder {
  ...
  async buildArgs(settings: NeptuneCoreSettings): Promise<string[]> {
    ...
    this.processCategory(args, 'network', settings.network, DEFAULT_NEPTUNE_CORE_SETTINGS.network);
    this.processCategory(args, 'mining', settings.mining, DEFAULT_NEPTUNE_CORE_SETTINGS.mining);
    this.processCategory(args, 'performance', settings.performance, DEFAULT_NEPTUNE_CORE_SETTINGS.performance);
    this.processCategory(args, 'security', settings.security, DEFAULT_NEPTUNE_CORE_SETTINGS.security);
    this.processCategory(args, 'data', settings.data, DEFAULT_NEPTUNE_CORE_SETTINGS.data);
    this.processCategory(args, 'advanced', settings.advanced, DEFAULT_NEPTUNE_CORE_SETTINGS.advanced);
   ...
   */
  private addFlag(args: string[], config: CLIFlagConfig, value: unknown): void {
   ...
   */
  private addComputedFlags(args: string[], settings: NeptuneCoreSettings): void {
   ...
   * Add positional arguments
   */
  private addPositionalArgs(args: string[], settings: NeptuneCoreSettings): void {
  ...
  private getArgCategory(arg: string): string {
    ...
    if (arg.includes('mine') || arg.includes('compose') || arg.includes('guess')) return 'Mining';
    if (arg.includes('proof') || arg.includes('mempool') || arg.includes('sync')) return 'Performance';
    if (arg.includes('ban') || arg.includes('security') || arg.includes('scan')) return 'Security';
    if (arg.includes('data') || arg.includes('import') || arg.includes('dir')) return 'Data';
