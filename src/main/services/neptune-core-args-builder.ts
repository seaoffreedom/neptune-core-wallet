...
import { CLI_FLAG_MAP, type CLIFlagConfig } from '../config/cli-flag-mapping';
...
export class NeptuneCoreArgsBuilder {
  ...
  async buildArgs(settings: NeptuneCoreSettings): Promise<string[]> {
    ...
    this.processCategory(
      args,
      'network',
      settings.network as unknown as Record<string, unknown>,
      DEFAULT_NEPTUNE_CORE_SETTINGS.network as unknown as Record<
        string,
        unknown
      >
    );
    this.processCategory(
      args,
      'mining',
      settings.mining as unknown as Record<string, unknown>,
      DEFAULT_NEPTUNE_CORE_SETTINGS.mining as unknown as Record<string, unknown>
    );
    this.processCategory(
      args,
      'performance',
      settings.performance as unknown as Record<string, unknown>,
      DEFAULT_NEPTUNE_CORE_SETTINGS.performance as unknown as Record<
        string,
        unknown
      >
    );
    this.processCategory(
      ...
      DEFAULT_NEPTUNE_CORE_SETTINGS.security as unknown as Record<
        string,
        unknown
      >
    );
    this.processCategory(
      ...
      DEFAULT_NEPTUNE_CORE_SETTINGS.data as unknown as Record<string, unknown>
    );
    this.processCategory(
      args,
      'advanced',
      settings.advanced as unknown as Record<string, unknown>,
      DEFAULT_NEPTUNE_CORE_SETTINGS.advanced as unknown as Record<
        string,
        unknown
      >
    );
  ...
  private addFlag(args: string[], config: CLIFlagConfig, value: unknown): void {
    if (config.type === 'boolean') {
      ...
      // Use equals format if specified
      if (config.useEqualsFormat) {
  ...
  private getArgCategory(arg: string): string {
    ...
    if (
      ...
      return 'Performance';
    if (arg.includes('ban') || arg.includes('security') || arg.includes('scan'))
      return 'Security';
    if (arg.includes('data') || arg.includes('import') || arg.includes('dir'))
      return 'Data';
