  ...
  private async validateBinaries(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        logger.error(`  Production: ${getRuntimeBinaryPath('neptune-core')}`);
    ...
    try {
      ...
      try {
        ...
        logger.error(`  Production: ${getRuntimeBinaryPath('neptune-cli')}`);
...
export function getNeptuneProcessManager(): NeptuneProcessManager {
  if (!_neptuneProcessManagerInstance) {
    ...
    logger.info('NeptuneProcessManager instance created (lazy initialization)');
  }
