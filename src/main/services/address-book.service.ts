...
export class AddressBookService {
  ...
  createEntry(data: {
    ...
    try {
      ...
      logger.info({ id: newEntry.id, title: newEntry.title }, 'Created entry');
      return newEntry;
    } catch (error) {
  ...
  importEntries(json: string, merge = false): number {
    try {
      ...
      if (!Array.isArray(importedEntries)) {
        throw new Error('Invalid JSON format: expected array of entries');
      }
      ...
      for (const entry of importedEntries) {
        ...
        const existingEntry = entries.find((e) => e.address === entry.address);
        if (existingEntry) {
          logger.warn({ address: entry.address }, 'Skipping duplicate address');
          continue;
        }
      ...
      this.store.set('entries', entries);
      logger.info({ importedCount, total: entries.length }, 'Imported entries');
