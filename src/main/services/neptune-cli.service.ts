...
import { getRuntimeBinaryPath, TIMEOUTS } from '../config/constants';
...
export class NeptuneCliService {
  ...
  constructor() {
    this.cliBinaryPath = getRuntimeBinaryPath('neptune-cli');
  ...
  private async validateBinaryPath(): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `neptune-cli binary not found at ${this.cliBinaryPath}. Please ensure binaries are available.`
      );
    }
  }
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        while (match !== null) {
          ...
          if (fullAddress.startsWith('[') && fullAddress.includes(']:')) {
            // IPv6 format: [::ffff:83.56.49.168]:9798
            const bracketEnd = fullAddress.indexOf(']:');
            const ipv6Address = fullAddress.substring(1, bracketEnd);
            port = parseInt(fullAddress.substring(bracketEnd + 2), 10);
