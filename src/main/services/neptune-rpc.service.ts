...
export class NeptuneRpcService {
  ...
  private async resilientCall<T>(
    ...
    const key = deduplicateKey || `${method}-${JSON.stringify(params || {})}`;
  ...
  async getMempoolTxCount(): Promise<number> {
    return this.resilientCall<number>('mempool_tx_count', undefined, 10000, {
      retries: 3,
  ...
  async getMempoolTxIds(): Promise<string[]> {
    return this.resilientCall<string[]>('mempool_tx_ids', undefined, 10000, {
      retries: 3,
