...
export class NeptuneRpcService {
  ...
  private async call<T>(
    ...
    return this.requestMutex.runExclusive(async () => {
      ...
      try {
        ...
        logger.info(
          {
            method,
            id: currentRequestId,
            cookie: this.cookie ? `${this.cookie.substring(0, 16)}...` : 'none',
            requestBody: request,
            headers,
          },
          'Making RPC call'
        ...
        logger.info({ method, id: currentRequestId }, 'RPC call successful');
        ...
        logger.debug({ method, id: currentRequestId }, 'RPC call successful');

        return response.result;
      } catch (error) {
        // Handle abort errors gracefully
        if (error instanceof Error && error.name === 'AbortError') {
          logger.info({ method, id: currentRequestId }, 'RPC call aborted');
          throw new Error(
   ...
   */
  async selectSpendableInputs(params: { amount: string }): Promise<unknown[]> {
   ...
   */
  async getMempoolTxKernel(params: { tx_kernel_id: string }): Promise<unknown> {
   ...
   */
  async getBlockDigestsByHeight(params: { height: number }): Promise<unknown> {
   ...
   */
  async upgradeTransaction(params: { tx_kernel_id: string }): Promise<boolean> {
  ...
  async getBlockDifficulties(params: {
    ...
    return this.call<Array<[number, number[]]>>('block_difficulties', params);
  }
