...
function getValidBinaryPath(binaryName: string): string {
  ...
  logger.info(
    `${mode} mode - using bundled path for ${binaryName}: ${bundledPath}`
  );
  return bundledPath;
}

/**
 * Get the correct binary path for the current environment (runtime version)
 * This function should be called at runtime, not at module load time
 * @param binaryName - The name of the binary (without extension)
 * @returns The full path to the binary, always using bundled binaries
 */
export function getRuntimeBinaryPath(binaryName: string): string {
  // Check if we're in a packaged app vs development
  const isPackaged =
    process.resourcesPath &&
    !process.resourcesPath.includes('node_modules/electron/dist');
 ...
 *
 * NOTE: These are now deprecated. Use getRuntimeBinaryPath() instead
 * to ensure paths are resolved at runtime, not module load time.
 */
