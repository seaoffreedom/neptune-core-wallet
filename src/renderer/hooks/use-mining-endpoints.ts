...
export function useMiningEndpoints() {
    ...
    async (nBlocks: number) => {
      ...
      try {
        ...
        if (
          ...
          showErrorToast('Mining is only available in regtest mode for safety');
        } else {
  ...
  const providePowSolution = useCallback(
    async (pow: unknown, proposalId: unknown) => {
      if (!isRegtest) {
        showErrorToast('PoW solution is only available in regtest mode');
        return;
      }
      ...
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : 'Unknown error';
        showErrorToast(`Failed to provide PoW solution: ${errorMessage}`);
        throw error;
      }
    },
