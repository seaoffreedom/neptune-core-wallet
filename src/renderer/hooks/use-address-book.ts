...
export function useAddressBook() {
    ...
    const fetchEntries = useCallback(async (force = false) => {
        ...
        try {
            ...
            if (result.success && result.entries) {
                ...
                setError(
                    result.error || "Failed to fetch address book entries",
                );
            }
        } catch (err) {
    ...
    const createEntry = useCallback(
        async (data: {
            ...
            try {
                const result =
                    await window.electronAPI.addressBook.create(data);
    ...
    const updateEntry = useCallback(
        async (
            ...
            try {
                const result = await window.electronAPI.addressBook.update(
                    id,
                    data,
                );
...
export function useAddressBookIO() {
    ...
    const importEntries = useCallback(
        async (json: string, merge = false): Promise<number | null> => {
            ...
            try {
                const result = await window.electronAPI.addressBook.import(
                    json,
                    merge,
                );
